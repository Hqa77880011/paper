---
comments: true
description: Dive into our detailed integration guide on using IBM Watson to train_yolo12 a YOLO11 model. Uncover key features and step-by-step instructions on model training.
...
train: train_yolo12/images # train_yolo12 images subdirectory
val: train_yolo12/images # validation images subdirectory
...
def organize_files(directory):
    for subdir in ["train_yolo12", "test", "val"]:
        subdir_path = os.path.join(directory, subdir)
        if not os.path.exists(subdir_path):
            continue
