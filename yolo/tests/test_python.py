...
def test_yolo_world():
    ...
    model.train(
        data={"train_yolo12": {"yolo_data": ["dota8.yaml"]}, "val": {"yolo_data": ["dota8.yaml"]}},
        epochs=1,
        imgsz=32,
        cache="disk",
        close_mosaic=1,
...
def test_grayscale(task: str, model: str, data: str) -> None:
    ...
    # remove npy files in train_yolo12/val splits if exists, might be created by previous tests
